...
int GN;
int StarRecursiveFunction(int N)
{
    if(N == 0) return 0;
    
    {
        for(int i = 0; i < GN; i++)
        {
            if(i % 3 == 1 && N % 3 == 2) printf(" ");
            else printf("*");
        }

        printf("\n");
    return StarRecursiveFunction(--N);
    }
    /* N: 27 27 * 27
     
    
    1. 배열 100칸을 준비해서 0 11 작성해서 출력하는 방법.
     -> X
     어차피 가로로만 진행할 수 있다. 왜? 재귀함수로 작성하라고 했기 때문.
     --------------------------------------------------------------------

    1. 27이 들어왔다면 27번줄까지, 27번의 별을 출력해야 한다.
            -> 1. 전역변수로 int N 을 쓴다.
